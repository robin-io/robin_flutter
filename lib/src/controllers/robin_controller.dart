...
class RobinController extends GetxController {
  ...
  DateTime lastAppResume = DateTime.now();

  DateTime lastSendAllInQueue = DateTime.now().subtract(Duration(seconds: 10));
  ...
  Future appResume({int? retry}) async {
    DateTime now = DateTime.now();
    if (now.difference(lastAppResume).inMilliseconds > 2000) {
      lastAppResume = now;
      try {
        robinConnect(retry: retry);
      } catch (e) {
        print(e);
      }
    }
  ...
  Future robinConnect({int? retry}) async {
    ...
    Future.delayed(const Duration(milliseconds: 500), () {
      connectionStartListen(retry: retry);
    });
  }

  connectionStartListen({int? retry}) async {
    ...
    robinStream = robinConnection?.stream.listen(
      (data) {
        ...
        if (data['is_event'] == null || data['is_event'] == false) {
          ...
          switch (data['name']) {
            case 'user.connect':
              if (data['value'] == currentUser?.robinToken) {
                rc.isConnected = true;
                retry = null;
                DateTime now = DateTime.now();
                if (now.difference(lastSendAllInQueue).inMilliseconds > 2000) {
                  lastSendAllInQueue = now;
      ...
      onError: (error) {
        ...
        int retries = retry ?? 0;
        if (retries < 4) {
          int duration = 250 * retries;
          if (duration > 5000) {
            duration = 5000;
          }
          print('Waiting $duration milliseconds before reconnecting');
          Future.delayed(Duration(milliseconds: duration), () {
            appResume(retry: retries += 1);
          });
        }
      },
      onDone: () {
  ...
  Future getConversations({bool? refresh}) async {
    try {
  ...
  void getMessages({bool? refresh}) async {
    try {
      ...
      Map conversationMessagesInQueue = {};
      if (currentConversation.value.id != null) {
        conversationMessagesInQueue =
            rc.messageQueue[currentConversation.value.id!] ?? {};
      }
      conversationMessages.value =
          toRobinMessage(response, conversationMessagesInQueue.values.toList());
      chatViewLoading.value = false;
    } catch (e) {
      chatViewLoading.value = false;
      showErrorMessage(e.toString());
  ...
  Map<String, RobinMessage> toRobinMessage(
      List messages, List undeliveredMessages) {
    ...
    for (Map message in undeliveredMessages) {
      RobinMessage robinMessage = RobinMessage.fromJson(message, false);
      if (!robinMessage.isRead && !robinMessage.sentByMe) {
        unreadMessages.add(robinMessage.id);
      }
      allMessages = {robinMessage.id: robinMessage, ...allMessages};
