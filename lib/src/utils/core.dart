...
class RobinCore {
  ...
  void sendTextMessage(Map message, String conversationId, String senderToken,
    ...
    try {
      if (rc.robinConnection!.closeCode == null || rc.isConnected) {
  ...
  sendAttachment(Map<String, dynamic> body, List<http.MultipartFile> files,
      Map message) async {
    try {
      if (rc.robinConnection!.closeCode == null || rc.isConnected) {
  ...
  replyWithAttachment(Map<String, dynamic> body, List<http.MultipartFile> files,
      Map message) async {
    try {
      if (rc.robinConnection!.closeCode == null || rc.isConnected) {
